#!/usr/bin/env python3
"""ft23.py — W77Q LUT Integrity Validation host runner.

Optionally deploys ft23-target.sh + common.sh to the board, then
...
Checks performed on-board:
  ...
  FT23.2 — Alignment (8-byte) and section bounds (0x1000 .. section end)
  FT23.3 — Duplicate flash offsets
  FT23.4 — Record overlaps
  FT23.5 — Magic bytes (0x574F424A = WOBJ) at each offset
...
ap = argparse.ArgumentParser(
    description='W77Q LUT integrity validation: deploy ft23-target.sh and run it')
ap.add_argument('--port',   default=PORT, help=f'Serial port (default: {PORT})')
ap.add_argument('--deploy', action='store_true',
                help='deploy ft23-target.sh + common.sh to /usr/bin before running')
args = ap.parse_args()
PORT = args.port
...
log("\n════════════════════════════════════════════════════")
log("  FT23 — W77Q LUT Integrity Validation")
log("════════════════════════════════════════════════════")
...
if args.deploy:
    ...
    for src, dst in [
        ...
        ('scripts/target/ft23-target.sh', '/usr/bin/ft23-target.sh'),
    ]:
...
# ── run ft23-target.sh ─────────────────────────────────────────────────────
log("\n[run] /usr/bin/ft23-target.sh\n" + "=" * 50)
t0 = time.time()
output = stream_command('ft23-target.sh 2>&1', timeout=600)
...
if summary_m:
    ...
    if passed == total:
        log(f"  FT23: PASS  ({passed}/{total} checks)")
        rc = 0
    else:
        log(f"  FT23: FAIL  ({passed}/{total} checks passed)")
        rc = 1
elif pass_m:
    log(f"  FT23: PASS  ({pass_m.group(1)}/{pass_m.group(2)} checks)")
    rc = 0
else:
    log("  FT23: FAIL  (no summary line found in output)")
    rc = 1
log("════════════════════════════════════════════════════")
