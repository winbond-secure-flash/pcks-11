#!/usr/bin/env python3
"""ft24.py — W77Q TEE Isolation and Data Protection host runner.

Optionally deploys ft24-target.sh + common.sh to the board, then
...
Checks performed on-board:
  FT24.1 — Token namespace isolation (cross-token object visibility)
  FT24.2 — Private object access control (login required)
  FT24.3 — Wrong PIN rejection
  FT24.4 — Data integrity roundtrip (write / read-back / byte compare)
  FT24.5 — Encryption at rest (plaintext not verbatim in flash)
  FT24.6 — TA UUID tagging in flash record
"""
...
ap = argparse.ArgumentParser(
    description='TEE isolation and data protection: deploy ft24-target.sh and run it')
ap.add_argument('--port',   default=PORT, help=f'Serial port (default: {PORT})')
ap.add_argument('--deploy', action='store_true',
                help='deploy ft24-target.sh + common.sh to /usr/bin before running')
args = ap.parse_args()
PORT = args.port
...
log("\n════════════════════════════════════════════════════")
log("  FT24 — W77Q TEE Isolation and Data Protection")
log("════════════════════════════════════════════════════")
...
if args.deploy:
    ...
    for src, dst in [
        ...
        ('scripts/target/ft24-target.sh', '/usr/bin/ft24-target.sh'),
    ]:
...
# ── run ft24-target.sh ─────────────────────────────────────────────────────
log("\n[run] /usr/bin/ft24-target.sh\n" + "=" * 50)
t0 = time.time()
output = stream_command('ft24-target.sh 2>&1', timeout=600)
...
if summary_m:
    ...
    if passed == total:
        log(f"  FT24: PASS  ({passed}/{total} checks)")
        rc = 0
    else:
        log(f"  FT24: FAIL  ({passed}/{total} checks passed)")
        rc = 1
elif pass_m:
    log(f"  FT24: PASS  ({pass_m.group(1)}/{pass_m.group(2)} checks)")
    rc = 0
else:
    log("  FT24: FAIL  (no summary line found in output)")
    rc = 1
log("════════════════════════════════════════════════════")
