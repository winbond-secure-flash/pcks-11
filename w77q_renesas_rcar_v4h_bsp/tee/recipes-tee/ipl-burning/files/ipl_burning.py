...
def main(argv):
    ...
    # W77Q sectors are write-protected; individual sector erase fails.
    # Chip erase (xcs) bypasses protection, so erase before writing.
    if SOC == "sparrow-hawk":
        test_dev.debug_flag = True
        print_debug("INFO", "Erasing entire SPI flash (xcs) — this takes ~220 seconds ...")
        test_dev.sendln()
        test_dev.wait(">", 0.5, "xcs")
        test_dev.wait("Clear OK?(y/n)", 0.2)
        test_dev.send("y")
        ret = test_dev.wait("complete!", 0.2, time_out=600)
        test_dev.debug_flag = False
        if ret != 0:
            print_debug("ERROR", "Chip erase timed out or failed")
            test_dev.close()
            exit(1)
        print_debug("INFO", "Chip erase complete")
        time.sleep(0.5)
